                    ...
                    <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><img src="@if (!($dato->firma == '')){{ asset('files/lavadero/'.$dato->firma) }} @endif" width="100"></td>
                                <td>
                                    @can('lavadero.edit')
                                    <a class="btn btn-info btn-sm" href="{{URL::action('LavaderoDesinfeccionController@edit',$dato->id)}}">
                                        <i class="fas fa-pencil-alt">
                                        </i>
                                        Edit
                                    </a>
                                    @endcan
                                    @can('lavadero.destroy')
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content modal-principal">
                                                <div class="modal-header">
                                                    <h4 class="modal-title">Eliminar registro de desinfeccion del lavadero</h4>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                        <span aria-hidden="true">×</span>
                                                    </button>
                                                </div>
                                                {{Form::Open(array('action'=>array('LavaderoDesinfeccionController@destroy',$dato->id),'method'=>'delete'))}}
                                                    <div class="modal-body">
                                                        <p class="text-center">Confirme si desea eliminar el registro de desinfeccion del lavadero con fecha {{$dato->fecha}}</p>
                                                    </div>
...
<script>
    $('#mes').change(function(){
        ...
        $('#generarPdf').attr('href','lavadero/generatePdf/'+$(this).val()+'?observacion='+$('#observacionText').val());
    });
    $('#observacionText').keyup(function(){
        $('#generarPdf').attr('href','lavadero/generatePdf/'+$('#mes').val()+'?observacion='+$('#observacionText').val());
    });
</script>
@endsection